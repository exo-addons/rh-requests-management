...
@NamedQueries({
        ...
        @NamedQuery(name = "vacatioRequestEntity.findActiveByUserId", query = "SELECT a FROM HRVacatioRequestEntity a where a.userId = :userId and (a.toDate > :currentDate or a.status  not in ('canceled','declined')) order by a.id desc"),
        @NamedQuery(name = "vacatioRequestEntity.findActive", query = "SELECT a FROM HRVacatioRequestEntity a where a.toDate > :currentDate or a.status  not in ('canceled','declined') order by a.id desc"),
        ...
        @NamedQuery(name = "vacatioRequestEntity.findActiveByValidator", query = "SELECT a FROM HRVacatioRequestEntity a, HRValidatorEntity b  where a.id=b.requestId and b.validatorUserId = :userId and (a.toDate > :currentDate or a.status  not in ('canceled','declined')) order by a.id desc"),
        @NamedQuery(name = "vacatioRequestEntity.countActive", query = "SELECT count (a.id) FROM HRVacatioRequestEntity a where a.toDate > :currentDate or a.status not in ('canceled','declined')"),
